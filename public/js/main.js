...
analyzeButton.addEventListener('click', async () => {
    ...
    try {
        ...
        console.warn('Backend server offline. Falling back to client-side mock parser.', error);
        if (progressInterval) clearInterval(progressInterval);
        progressFill.style.width = '100%';
        
        const mockData = generateMockResumeData(selectedFile.name);
        currentResumeData = mockData;
        
        progressText.textContent = 'Analysis complete (Offline Demo Mode)';
